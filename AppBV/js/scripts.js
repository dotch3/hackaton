	...
	if (jsonObj !== undefined) {
		for (var i = 0; i < jsonObj.length; i++) {
			...
			if (jsonObj[i].descricao !== undefined) {
				...
				if (document.getElementsByClassName("pub_urlDetalheItem1")[i] !== undefined)
				{
					document.getElementsByClassName("pub_urlDetalheItem1")[i].href = urlDetalheItem;
				}
				
				if (document.getElementsByClassName("pub_urlDetalheItem2")[i] !== undefined)
				{
					document.getElementsByClassName("pub_urlDetalheItem2")[i].href = urlDetalheItem;
				}

				if (document.getElementsByClassName("pub_urlDetalheItem3")[i] !== undefined)
				{
...
/*
    Session functions
*/
function setSession(cnome, cvalor, exminutos = 10) {
	if (exminutos > 0) {
		let expirationDate = new Date(new Date().getTime() + (60000 * exminutos))
		let newValue = {
			idUser: cvalor,
			expirationDate: expirationDate
		}		
		window.sessionStorage.setItem(cnome, JSON.stringify(newValue));
	}
	else
	{
		window.sessionStorage.removeItem(cnome);
	}
}

function getSession(cnome) {
	let stringValue = window.sessionStorage.getItem(cnome)
    if (stringValue !== null) {
      let value = JSON.parse(stringValue)
        let expirationDate = new Date(value.expirationDate)
        if ((value.idUser !== undefined) && (expirationDate > new Date())) {
          return value.idUser
        } else {
		  window.sessionStorage.removeItem(cnome)
...
function sendCredentials(email, senha) {
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState === 4 && xhr.status === 200) {
			...
			$('.modal-carregando').modal('hide');
			$('.modal-login-invalido').modal('hide');
...
$('#btnPesquisar').on('click', function (event) {
	...
	var pesq = $('#txtPesquisar').val().trim();
...
function pagMinhasPub() {
	...
	var idUser = getSession('appBV');
	if (idUser != '') {
		runWS('http://54.163.66.128:8080/items/findItemProprietario/' + idUser, function (text) {
			var data = JSON.parse(text);
			if (data.length > 0) {
				popularItens(data);
